...
class UserSession(BaseModel, table=True):
    ...
    def get_expiry_time(cls) -> datetime:
        ...
        return datetime.utcnow() + datetime.timedelta(hours=24)  # Default 24 hours
...
class Image(BaseModel, table=True):
    ...
    timestamp: Optional[datetime.datetime] = Field(default_factory=datetime.datetime.now(datetime.UTC))
...
class AlbumPhoto(BaseModel, table=True):
